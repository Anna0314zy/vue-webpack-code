...
export default {
  ...
  profileAuth(to, from, next) {
    ...
    const formatList = (authRoutes, auths) => authRoutes.filter((route) => {
    ...
    if (store.state.hasPermisson && store.state.user && store.state.user.menuList) {
      if (!store.state.menuPermisson) {
        const auths = store.state.user.menuList.map(item => item.auth);
        const newRoutes = formatList(auth, auths);
